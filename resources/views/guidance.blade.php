...
<section class="relative overflow-hidden py-20 md:py-28">
    ...
    <div class="absolute inset-0 bg-cover bg-center bg-no-repeat opacity-[0.10]" style="background-image: url('{{ asset('images/gvc.png') }}');"></div>
    ...
    <div class="max-w-4xl mx-auto px-6">
        ...
        <div class="mb-10 fade-up">
            <div class="flex items-center gap-3 mb-4">
                ...
                @php $pendingCount = $invitations->where('status', 'pending')->count(); @endphp
                @if($pendingCount > 0)
            ...
            <div class="space-y-2">
                ...
                <div class="bg-white rounded-xl border shadow-sm px-4 py-3 flex items-center gap-3 flex-wrap
                            {{ $inv->status === 'pending' ? 'border-amber-200' : ($inv->status === 'accepted' ? 'border-green-200' : 'border-red-200') }}">

                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-slate-800 truncate">
                            {{ $inv->session_date?->format('F d, Y') ?? 'Date TBD' }}
                            @if($inv->timeSlot)
                                <span class="text-slate-400 font-normal">&mdash; {{ $inv->timeSlot->name }}</span>
                            @endif
                        </p>
                        <p class="text-xs text-slate-400 truncate">
                            {{ $inv->personnel ? trim($inv->personnel->first_name . ' ' . $inv->personnel->last_name) : 'Counselor TBA' }}
                            @if($inv->purpose) &mdash; {{ Str::limit($inv->purpose, 60) }} @endif
                        </p>
                    </div>

                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold flex-shrink-0
                                 {{ $inv->status === 'pending' ? 'bg-amber-100 text-amber-700' : ($inv->status === 'accepted' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600') }}">
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        <form method="POST" action="{{ route('referral.invitation.respond', $inv->id) }}">
                            @csrf
                            @method('PATCH')
        ...
        <div class="mb-10 fade-up delay-1">
            <div class="flex items-center gap-3 mb-4">
                ...
                @php $upcomingCount = $appointments->whereIn('status', ['pending', 'approved'])->count(); @endphp
                @if($upcomingCount > 0)
            ...
            <div class="space-y-2">
                ...
                <div class="bg-white rounded-xl border shadow-sm px-4 py-3 flex items-center gap-3 flex-wrap {{ $statusColor['border'] }}">

                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-slate-800 truncate">
                            {{ $appt->counseling_date->format('F d, Y') }}
                            @if($appt->timeSlot)
                                <span class="text-slate-400 font-normal">&mdash; {{ $appt->timeSlot->name }}</span>
                            @endif
                        </p>
                        <p class="text-xs text-slate-400 truncate">
                            @if($appt->modeOfCounseling){{ $appt->modeOfCounseling->name }}@endif
                            @if($appt->supportNeeded) &mdash; {{ $appt->supportNeeded->name }}@endif
                            @if(!empty($appt->concern)) &mdash; {{ Str::limit($appt->concern, 50) }}@endif
                        </p>
                    </div>

                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold flex-shrink-0 {{ $statusColor['badge'] }}">
                        @if($appt->status === 'pending' && $statusColor['dot'])
                            <span class="w-1.5 h-1.5 rounded-full {{ $statusColor['dot'] }} animate-pulse"></span>
                        @elseif($appt->status === 'approved')
                            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                        @elseif(in_array($appt->status, ['cancelled', 'rejected']))
                            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                    ...
                    <div id="cancel-modal-{{ $appt->id }}"
                        ...
                        <div class="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-sm p-6">
                            ...
                            <p class="text-sm text-slate-500 text-center mb-1">
                                {{ $appt->counseling_date->format('F d, Y') }}
                                @if($appt->timeSlot) &mdash; {{ $appt->timeSlot->name }} @endif
                            </p>
                            ...
                            <div class="flex gap-3">
                                ...
                                <form method="POST" action="{{ route('guidance.appointment.cancel', $appt->id) }}" class="flex-1">
                                    @csrf
                                    @method('PATCH')
        ...
        <div class="grid gap-6 sm:grid-cols-2">
            ...
            <a href="{{ route('guidance.appointment') }}"
