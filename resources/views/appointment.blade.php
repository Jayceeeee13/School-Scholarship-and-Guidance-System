...
<section style="padding:40px 16px 64px;">
    <div style="max-width:720px;margin:0 auto;">
        ...
        <div class="form-card">
            ...
            <form method="POST" action="{{ route('appointments.post') }}"
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                        ...
                        <div>
                            ...
                            <input type="text" name="last_name"
                                   ...
                                   {{ $student?->last_name ? 'readonly' : '' }}
                                   class="field-input {{ $errors->has('last_name') ? 'is-invalid' : '' }} {{ $student?->last_name ? 'is-readonly' : '' }}">
                        ...
                        <div>
                            ...
                            <input type="text" name="first_name"
                                   ...
                                   {{ $student?->first_name ? 'readonly' : '' }}
                                   class="field-input {{ $errors->has('first_name') ? 'is-invalid' : '' }} {{ $student?->first_name ? 'is-readonly' : '' }}">
                        ...
                        <div>
                            ...
                            <input type="text" name="middle_name"
                                   ...
                                   {{ $student?->middle_name ? 'readonly' : '' }}
                                   class="field-input {{ $errors->has('middle_name') ? 'is-invalid' : '' }} {{ $student?->middle_name ? 'is-readonly' : '' }}">
                        ...
                        <div>
                            ...
                            <input type="text" name="course_and_year"
                                   ...
                                   {{ $courseAndYear ? 'readonly' : '' }}
                                   class="field-input {{ $errors->has('course_and_year') ? 'is-invalid' : '' }} {{ $courseAndYear ? 'is-readonly' : '' }}">
                            @error('course_and_year')<p class="field-error">{{ $message }}</p>@enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="tel" name="contact_no"
                                   ...
                                   class="field-input {{ $errors->has('contact_no') ? 'is-invalid' : '' }}">
                        ...
                        <div>
                            ...
                            <input type="text" name="present_address"
                                   ...
                                   class="field-input {{ $errors->has('present_address') ? 'is-invalid' : '' }}">
                            @error('present_address')<p class="field-error">{{ $message }}</p>@enderror
                        </div>

                        {{-- Counseling Date — custom calendar picker --}}
                        <div style="grid-column:1/-1;" x-data="calendarPicker()" @click.outside="close()">
                            ...
                            <div class="calendar-wrapper">
                                ...
                                <button type="button"
                                        @click="toggle()"
                                        :class="['calendar-input-btn', open ? 'open' : '', '{{ $errors->has('counseling_date') ? 'is-invalid' : '' }}']">
                            ...
                            @error('counseling_date')<p class="field-error">{{ $message }}</p>@enderror
                        </div>
                ...
                <div x-data="timeSlotPicker()" x-init="init()">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;">
                        ...
                        <div>
                            ...
                            <select name="time_slot_id"
                                    ...
                                    :class="['field-input', !selectedDate ? 'is-disabled' : '', '{{ $errors->has('time_slot_id') ? 'is-invalid' : '' }}']"
                                    x-ref="slotSelect">
                            ...
                            @error('time_slot_id')<p class="field-error">{{ $message }}</p>@enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="support_needed_id"
                                    class="field-input {{ $errors->has('support_needed_id') ? 'is-invalid' : '' }}">
                                <option value="">Select Support Needed</option>
                                @foreach($supportNeeds as $support)
                                    <option value="{{ $support->id }}"
                                        {{ old('support_needed_id') == $support->id ? 'selected' : '' }}>
                                        {{ $support->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('support_needed_id')<p class="field-error">{{ $message }}</p>@enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="mode_of_counseling_id"
                                    class="field-input {{ $errors->has('mode_of_counseling_id') ? 'is-invalid' : '' }}">
                                <option value="">Select Mode</option>
                                @foreach($modeOfCounselings as $mode)
                                    <option value="{{ $mode->id }}"
                                        {{ old('mode_of_counseling_id') == $mode->id ? 'selected' : '' }}>
                                        {{ $mode->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('mode_of_counseling_id')<p class="field-error">{{ $message }}</p>@enderror
                        </div>
                    ...
                    <div style="margin-top:16px;"
                         x-data="notesField('{{ old('concern') }}', 9999)">
                        ...
                        <div style="position:relative;">
                            <textarea
                                ...
                                class="field-input {{ $errors->has('concern') ? 'is-invalid' : '' }}"
                        ...
                        <p style="margin-top:4px;font-size:0.7rem;"
                           ...
                           x-text="hint"></p>
                        @error('concern')<p class="field-error" role="alert">{{ $message }}</p>@enderror
                    </div>
                </div>
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;padding-top:8px;border-top:1px solid var(--slate-border);flex-wrap:wrap;gap:12px;">
                    <a href="{{ route('guidance') }}" class="btn-back">
...
<script>
    ...
    const INACTIVE_DATES = @json($inactiveDates);

    // ── All time slots from server ───────────────────────────────────
    const ALL_SLOTS = @json($timeSlots->map(fn($s) => ['id' => $s->id, 'name' => $s->name]));
    ...
    function calendarPicker() {
        ...
        return {
            ...
            selectedValue: '{{ old('counseling_date', '') }}',
            selectedLabel: @if(old('counseling_date')) '{{ \Carbon\Carbon::parse(old('counseling_date'))->format('F d, Y') }}' @else null @endif,
    ...
    function timeSlotPicker() {
        return {
            ...
            init() {
                ...
                const oldDate = '{{ old('counseling_date', '') }}';
                if (oldDate) {
