    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-3 gap-4">
            @foreach ([
                ...
                ['label' => 'Categories Used', 'value' => $stats['categories'],  'icon' => 'heroicon-o-tag',           'color' => 'text-green-600',  'bg' => 'bg-green-50'],
            ] as $stat)
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-[1fr_auto_auto]">
            ...
            <button wire:click="openCreate"
                ...
                class="inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold shadow hover:opacity-90 transition whitespace-nowrap">
        ...
        {{-- QUESTIONS LIST --}}
@php $catCounters = []; $globalCounter = 0; @endphp
<div class="space-y-3">
    @forelse ($this->getQuestions() as $q)
    @php
        $globalCounter++;
        $categoryId = $q->exam_category_id ?? 'none';
        if (!isset($catCounters[$categoryId])) {
            $catCounters[$categoryId] = 0;
        }
        $catCounters[$categoryId]++;
        $displayNumber = $filterCategory ? $catCounters[$categoryId] : $globalCounter;
    @endphp
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm hover:shadow-md transition-shadow duration-200">
        <div class="flex items-start gap-4 p-4">
            ...
            <div class="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-200 bg-white py-16 text-center">
                ...
                <p class="text-base font-semibold text-gray-500">No questions yet</p>
                <p class="mt-1 text-sm text-gray-400">Add your first question to get started.</p>
                <button wire:click="openCreate"
                    style="background-color: #16a34a !important; color: #ffffff !important;"
                    class="mt-4 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold hover:opacity-90 transition whitespace-nowrap">
                    @svg('heroicon-o-plus-circle', 'w-4 h-4')
                    Add First Question
                </button>
            </div>
        ...
        <div class="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white shadow-2xl">
            ...
            <div class="sticky top-0 z-10 flex items-center justify-between border-b border-gray-100 bg-white px-6 py-4">
                ...
                <button wire:click="$set('showModal', false)"
                    class="rounded-lg p-2 text-gray-400 hover:bg-gray-100 transition">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea wire:model="question" rows="4"
                        ...
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 resize-none"></textarea>
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center gap-3 rounded-xl border px-4 py-3 transition
                            {{ $choice['is_correct'] ? 'border-green-300 bg-green-50' : 'border-gray-200 bg-gray-50' }}">
                            ...
                            <button type="button" wire:click="toggleCorrect({{ $index }})"
                                class="shrink-0 rounded-full p-1 transition
                                    {{ $choice['is_correct'] ? 'text-green-600 hover:text-green-700' : 'text-gray-300 hover:text-green-500' }}">
            ...
            <div class="sticky bottom-0 flex items-center justify-end gap-3 border-t border-gray-100 bg-white px-6 py-4">
                ...
                <button wire:click="save"
                    style="background-color: #16a34a !important; color: #ffffff !important;"
                    class="inline-flex items-center gap-2 rounded-lg px-5 py-2 text-sm font-semibold hover:opacity-90 transition whitespace-nowrap">
        ...
        <div class="relative w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl text-center">
            ...
            <div class="flex gap-3">
                ...
                <button wire:click="deleteQuestion"
                    ...
                    class="flex-1 rounded-lg py-2 text-sm font-semibold hover:opacity-90 transition">
                    Yes, Delete
                </button>
            </div>
        </div>
