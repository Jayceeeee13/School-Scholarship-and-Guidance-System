    ...
    public function getTabs(): array
    {
        return [
            ...
            'endorsed' => Tab::make('Endorsements')
                ...
                ->badgeColor('info')
                ->badge(fn () => Referrals::whereNull('archived_at')
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('endorsed_by')
                    ->label('Endorsed By')
                    ->getStateUsing(
                        fn ($record) => $record->endorsement?->personnel
                            ? trim(
                ...
                Tables\Columns\TextColumn::make('invitation_time_slot')
                    ->label('Time Slot')
                    ->getStateUsing(
                        fn ($record) => $record->invitation?->timeSlot?->name ?? '-'
                    )
                    ->hidden(fn () => $this->activeTab !== 'invited'),
                ...
                Tables\Columns\TextColumn::make('invitation_counselor')
                    ->label('Counselor')
                    ->getStateUsing(
                        fn ($record) => $record->invitation?->personnel
                            ? trim(
                                "{$record->invitation->personnel->first_name} " .
                                "{$record->invitation->personnel->middle_name} " .
                                "{$record->invitation->personnel->last_name}"
                            )
                            : '-'
                    )
                    ->hidden(fn () => $this->activeTab !== 'invited'),
            ...
            ->actions([
                Tables\Actions\ActionGroup::make([
                    ...
                    Tables\Actions\EditAction::make()
                        ->url(
                            fn ($record) => ReferralsResource::getUrl(
                                'edit',
                                [
                                    'record' => $record->id,
                        ...
                        ->action(
                            function (Referrals $record): void {
                                ...
                                Notification::make()
                                    ...
                                    ->body(
                                        "The referral for {$record->name} has been approved."
                                    )
                                    ->success()
                                    ->send();
                            }
                    ...
                    Tables\Actions\Action::make('complete')
                        ...
                        ->color('info')
                        ->requiresConfirmation()
                        ->modalHeading('Complete Referral')
                        ->modalDescription(
                            'Are you sure you want to mark this referral as completed?'
                        )
                        ...
                        ->action(
                            function (Referrals $record): void {
                                ...
                                $this->logCustomActivity(
                                    ...
                                    'referrals',
                                    'completed',
                                    "Completed referral for {$record->name}"
                                );
                                ...
                                Notification::make()
                                    ...
                                    ->body(
                                        "The referral for {$record->name} has been marked as completed."
                                    )
                                    ->success()
                                    ->send();
                            }
                        ...
                        ->infolist([
                            ...
                            \Filament\Infolists\Components\Section::make(
                                ...
                                ->schema([
                                    ...
                                    \Filament\Infolists\Components\TextEntry::make(
                                        'endorsement.received_by'
                                    )
                                        ->label('Received By')
                                        ->default('-'),

                                    \Filament\Infolists\Components\TextEntry::make(
                                        'endorsement.receive_date'
                                    )
                                        ->label('Date Received')
                                        ->formatStateUsing(
                                            fn ($state) => $state
                ...
                Tables\Actions\ActionGroup::make([
                    ...
                    Tables\Actions\Action::make('mark_accepted')
                        ...
                        ->modalDescription(
                            'Are you sure you want to mark this invitation as accepted?'
                        )
                        ...
                        ->action(
                            function (Referrals $record): void {
                                ...
                                Notification::make()
                                    ...
                                    ->body(
                                        "The invitation for {$record->name} has been marked as accepted."
                                    )
                                    ->success()
                                    ->send();
                            }
                    ...
                    Tables\Actions\Action::make('mark_declined')
                        ...
                        ->modalDescription(
                            'Are you sure you want to mark this invitation as declined?'
                        )
                        ->modalSubmitActionLabel('Yes, Decline')
                        ->visible(
                            fn (Referrals $record): bool =>
                                $record->invitation?->status !== 'declined'
                        )
                        ->action(
                            function (Referrals $record): void {
                                $record->invitation()?->update([
                                ...
                                Notification::make()
                                    ...
                                    ->body(
                                        "The invitation for {$record->name} has been marked as declined."
                                    )
                                    ->danger()
                                    ->send();
                            }
